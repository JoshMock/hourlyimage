...
def list_directories(year=None, month=None):
    """
    ...
    if year:
        ...
        if month:
            ...
            path = "%s/%s/%s" % (hourlyimage.app.config["IMAGE_LOCATION_DIR"],
                    year, month)
        else:
            path = "%s/%s" % (hourlyimage.app.config["IMAGE_LOCATION_DIR"],
                    year)
    else:
...
def list_images(year, month, day):
    """
    ...
    path = "%s/%s/%s/%s" % (hourlyimage.app.config["IMAGE_LOCATION_DIR"], year,
            month, day)
    url = "%s/%s/%s/%s" % (hourlyimage.app.config["IMAGE_LOCATION_URL"], year,
            month, day)
